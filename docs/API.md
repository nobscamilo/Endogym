...
- `GET /api/studio-data` devuelve `overrides.mode:'authenticated'`, `dataStatus` y `planStatus`. Todas las secciones de muestra quedan reemplazadas por datos reales, `null` o arrays vacíos. Un bloque que no contiene la fecha civil actual es `stale`; no se reutiliza su primer día. Los vídeos de `todaySession` y `library` se resuelven desde el mapa curado vigente por `exercise.id`, no desde un `videoEmbedId` obsoleto persistido en el plan. Cada ejercicio expone `yt` solo si existe embed exacto y `videoUrl` como búsqueda específica; ya no existe una sección `discover` editorial en el contrato. Si la sesión contiene ejercicios, `todaySession.guidedWarmup` expone preparación dinámica original de 5/10/15 min y `todaySession.guidedMobility` recuperación de 5/10/15/20 min; ambas se derivan de `category` e incluyen contexto, seguridad, material y movimientos temporizados.
- `progress.sessionsPlan` representa las sesiones planificadas de la semana civil actual, `sessionsDue` las ya vencidas y `sessionsDone` solo las fechas vencidas que tienen un registro completado fusionado. `adherence` es `sessionsDone/sessionsDue`; es `null` si aún no existe ninguna sesión vencida. Workouts antiguos o días extra no planificados no cuentan como adherencia del plan vigente.
...
**Alineación con objetivos (implementada localmente el 19 jun 2026, pendiente de deploy):** el informe incluye `goalAlignment` y recibe `goalProgress` (meta SMART, valor actual, tendencia, fecha y `onTrack`). Para carrera añade FCmáx/rango Z2 del modelo de la app, zona real vs prescrita, adherencia a tirada larga/calidad, eficiencia y predicción basada en esfuerzos reales. El fallback ramifica por objetivo. La firma cacheada `v2` se invalida no solo por nuevos entrenos, sino también por cambios de objetivo/plan, edición de datos, métricas y comidas.
