...
- **Coach IA ("Pregúntale al coach" + guía contextual):** el bundle define `window.claude.complete` → `POST /api/coach-chat` (`src/app/api/coach-chat/route.js`) enviando `{ message }`. El servidor mantiene la persona única en `coachPersona.js` y la envía como `systemInstruction` de la API (canal separado del turno de usuario, que solo lleva datos: contexto real, RAG, memoria y pregunta); evalúa red flags deterministas antes de Gemini/rate limit y usa el transporte Gemini existente (`gemini-2.5-flash`) en el flujo normal. Requiere auth y aplica rate limit persistente `coach-chat` (20/h por defecto). Si falla, `coach.jsx` explica la causa real (límite por hora con su espera, sesión caducada, coach no configurado) y solo cae al mensaje genérico ante fallos desconocidos o de red; esas burbujas no piden feedback 👍👎. El banner estático ya no se presenta como IA live ni inventa métricas personales; la IA live es el modal de preguntas. En móvil, el modal se monta por portal en `document.body`, bloquea el scroll de fondo y mantiene el input dentro del viewport.
