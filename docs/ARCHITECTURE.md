...
- En esa lectura, los ejercicios reales del día (`exercise.category`) son la fuente de verdad para recomputar calentamiento/enfriamiento y para construir `guidedWarmup`/`guidedMobility`; un protocolo genérico u obsoleto guardado en el bloque no prevalece sobre la selección actual.
- La adherencia visible se calcula sobre fechas: intersección de sesiones planificadas ya vencidas de la semana civil con workouts completados colapsados por día. Nunca se usa el número bruto de documentos ni historial anterior al periodo como numerador.
...
- Método híbrido (fuerza en circuito): NO es una modalidad, es un formato. `prefersHybridCircuit` (booleano en Perfil, toggle de la encuesta) o el objetivo `recomposition` (sesgo de 1 sesión) hacen que el planner reorganice las últimas sesiones de fuerza de la semana como circuito (`workout.hybridCircuit`: rest 25 s, carga ×0.85, RPE 6-7); la primera sesión de fuerza semanal se conserva tradicional. Excluido en modalidades de resistencia (`hybrid_run_gym`/`running`/`cycling`) y mindbody. El RAG (`guidelinesRetriever.js`) refleja la misma condición con `planWillIncludeHybridCircuit`.
- RAG de directrices (`guidelinesRetriever.js`): recuperación semántica sobre `guideline_passages` (7.128 pasajes, 768 dims, findNearest COSINE) con fallback léxico a `guidelines`. Dos salvaguardas de calidad (12 jul 2026): (1) FILTRO DE BIBLIOGRAFÍA — los pasajes están etiquetados EN LOS DATOS con `isReference`/`refScore` (backfill `scripts/backfill_reference_flags.mjs` sobre los 7.128 existentes — 1.592 son bibliografía, 22% — y la ingesta `embed_guidelines.mjs` etiqueta los nuevos en origen); el retriever sobre-recupera (`PASSAGE_FETCH_LIMIT=40`), descarta por flag (heurística `isBibliographyPassage` solo como fallback para pasajes sin etiquetar) y deja `PASSAGE_LIMIT=12` útiles. (2) MULTI-QUERY híbrido — solo en weekly-plan (no chat, por latencia): cuando el plan incluirá circuito, se lanzan 3 sub-consultas (fuerza/HIIT/concurrente) además de la principal y se fusionan por id de pasaje. Verificado con sondas contra Firestore real en `scratch/rag-*.mjs`.
- `studio-swap` puede cambiar el foco muscular de la sesión de hoy (`scope:'focus'`) sin regenerar el bloque. La lógica vive en `planner.js` (`buildSessionFocusChange`/`listSessionFocusChangeOptions`) y bloquea conflictos con días adyacentes antes de persistir el plan. **Excepción retroactiva (17 jul 2026):** si el cambio es sobre un día PASADO (`date !== today`, vía editor retroactivo), esos guardarraíles de equilibrio semanal/adyacencia dejan de BLOQUEAR (se informan como nota) — editar un día que ya ocurrió es registrar lo realmente hecho, no planificar. Se activa con el flag `retroactive` (lo pasan `studio-swap` y `session-for-date`); el aviso clínico de conversión se conserva y hoy/futuro no cambian.
