...
- [ ] **Modalidad "Solo correr" en "Dónde y cómo entrenas" (pedido por el usuario, 26 jul 2026).** Hoy `AV_EQUIP` (screen-more.jsx:594) ofrece Gimnasio, Correr + gym, Flexible, TRX y Casa: quien solo corre no tiene opción y acaba eligiendo "Correr + gym", que le prescribe fuerza que no quiere. Lo que hay que revisar antes de añadir la tarjeta: `resolveTrainingModality` en `core/planner.js` ya conoce `running` como modalidad de resistencia, pero el planner la excluye a propósito de varias ramas (ver comentarios de planner.js:1098 y 1171) — hay que comprobar qué genera hoy un perfil `running` puro y si el bloque de 21 días sale coherente (tiradas, calidad, Z2) sin sesiones de gimnasio. Tocan también: prescripción de ritmos (`core/running.js` ya la tiene), el contexto del chat (`modality === 'running'` ya está contemplado en coach-chat) y el objetivo de carrera. NO es solo añadir una tarjeta a la lista.
