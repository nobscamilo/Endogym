...
- [ ] **Fricción operativa (punto 4).** (a) **RESUELTO A MEDIAS (28 jul 2026): el alias `endogym.vercel.app` NO se promociona solo de forma fiable.** Observado en directo: el deploy `dpl_GwByb5hT` (commit 00ce8b4) llegó a READY en 46 s y, tres minutos y seis sondas después, el alias seguía apuntando al deploy ANTERIOR; `GET /api/coach-chat` y el hash del bundle lo confirmaban. Se arregla en 2 s con `npx vercel promote <dpl_id> --scope=team_uMqfENLajmONg5snqLTiYbHu`. OJO, el comportamiento es INCONSISTENTE: el deploy del día anterior (`dpl_Cd8PAwVs`) sí acabó promocionándose solo a los ~2-3 min sin intervención, y por eso se atribuyó erróneamente a "propagación de CDN". Queda por investigar la causa (¿staged rollout? ¿protección de deployment? ¿ajuste del proyecto?), pero el procedimiento operativo es claro: **tras cada deploy, comprobar a qué deploy resuelve el alias y promocionar si no coincide**; no basta con ver READY ni con esperar. (b) Verificar qué cubre exactamente el cron de backup semanal (`/api/backup`) y RESTAURARLO una vez: un backup no probado es una hipótesis.
- [x] **Alerta de gasto de IA (deuda menor del punto 1) — HECHO 25 jul.** `ai_cost_report.mjs --alert` sale con código 1 si se supera un umbral, listo para tarea programada. Vigila el coste diario (`AI_COST_DAILY_ALERT_USD`, $1 por defecto) y además la SALUD de cada flujo: ratio de fallback heurístico, errores, respuestas truncadas y votos negativos, con muestra mínima para no avisar por ruido. Motivo de mirar más que el dinero: un flujo roto que cae al heurístico cuesta MENOS, así que el gasto solo lo esconde. Nuevo `scripts/coach_feedback_report.mjs` agrega los 👍👎 por endpoint y por respuesta (hash), que hasta ahora se guardaban y no leía nadie.
- [x] **Modalidad "Solo correr" en "Dónde y cómo entrenas" — HECHO 26 jul 2026.** El planner ya la soportaba (`MODALITY_TEMPLATES[running]`); lo que faltaba era la tarjeta en `AV_EQUIP` y, sobre todo, que la lista blanca de `/api/studio-availability` la aceptara: sin eso la encuesta la descartaba EN SILENCIO. La tarjeta dice "Carrera + 1 fuerza preventiva" y no "solo carrera" porque la plantilla incluye fuerza (ACSM, prevención de lesiones): no se vende una cosa y se entrega otra. La pregunta de objetivo de carrera pasa a mostrarse también aquí, y el formato de circuito híbrido se oculta (el planner excluye las modalidades de resistencia por interferencia). Verificado que un bloque de 21 días sale coherente: 12 aeróbicas con prescripción por tipo (suave/calidad/tirada larga), 3 de fuerza, 6 de recuperación. +7 tests de contrato del bloque y 1 que ata `AV_EQUIP` con la lista blanca para que no se repita el fallo silencioso.
- [ ] **`weekly-plan` cae al heurístico demasiado — EN OBSERVACIÓN desde el 26 jul 2026.** Histórico: 48-56% de fallback. Investigado: los logs de Vercel en plan Hobby duran **1 hora**, así que la causa ya no existía. Reproducido en local con perfil y plan reales (`scratch`, sonda de 3 llamadas): **3/3 éxito en 9-11 s**, muy dentro del timeout de 18 s, JSON válido y 6-7 ajustes. O sea: hoy no falla. La hipótesis más probable es que el histórico esté dominado por el periodo ANTERIOR al fix de timeout del 20 jul (con 10 s de tope y llamadas de 9-11 s, fallaría ~la mitad — encaja con el 48%), pero quedan 2 días posteriores con fallback que no se pueden explicar sin datos. Ya no hace falta arqueología: los contadores `fbTimeout`/`fbHttp`/`fbParse`/`fbInvalid` registran el motivo y el aviso de `ai_cost_report.mjs` lo imprime. **Cerrar este punto cuando haya datos nuevos**: si el ratio baja solo, era el timeout viejo; si no, el motivo ya vendrá dicho.
- [x] **Evals del plan semanal — HECHO 2 ago 2026.** `scripts/weekly_plan_evals.mjs`, hermano del de chat pero midiendo lo que el auditor HACE, no lo que dice: sus `structuredAdjustments` se aplican al plan de verdad. La métrica que manda es cuántos ACABAN APLICÁNDOSE. Encontró a la primera que solo el 38% llegaba al plan (ver punto siguiente). Tras el arreglo: 22/22 en tres ejecuciones.
- [x] **Ajustes del coach que se perdían en silencio — CORREGIDO 2 ago 2026.** `applyCoachAdjustments` emparejaba el día mirando el NOMBRE antes que la FECHA, y en un bloque de 21 días cada día de la semana aparece TRES veces: `.find()` devolvía siempre el primer martes, el ejercicio no coincidía y el ajuste se descartaba sin avisar. Nuevo `findPlanDayByLabel` (fecha > título > nombre) + 9 tests deterministas. Además el prompt ahora exige fecha completa en `day` y limita los ajustes a ejercicios con prescripción por repeticiones (los de tiempo no admiten carga ni series). De 38% a 100% de ajustes aplicados.
- [ ] **Caché de prefijo en el chat.** `coach-chat` muestra 0% de entrada cacheada frente al 29% de `studio-nutrition`. Con la persona ya en `systemInstruction` el prefijo es estable, pero son ~250 tokens: por debajo del mínimo que Gemini exige para la caché implícita. Si se quiere aprovechar, el candidato es el bloque científico grande del plan semanal, no el chat.
...
- [x] Auditar semánticamente los vídeos: 55 asociaciones exactas / 49 vídeos únicos resolubles por oEmbed; no compartir entre movimientos o implementos distintos, aplicar el mapa vigente también a planes guardados y usar búsqueda específica/fallback local cuando no exista vídeo exacto. Desplegado el 20 jun 2026 en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`.
- [x] Retirar el feed ficticio de “Sigue aprendiendo” y el reproductor simulado: Inicio deriva vídeos verificados de la sesión real; Biblioteca abre iframe real o búsqueda específica externa. Bundle `bb2659ac92`, 339 tests y Playwright de producción verdes; desplegado el 20 jun 2026.
...
- [x] **#3 Check-in rápido por grupo muscular antes de cambiar foco** — implementado y desplegado (ver sección "Registro de sesiones e historial" y la sesión del 15 jun noche-3). Modula la sesión nueva si la zona dolorida coincide con el grupo elegido.
- [x] **#4 Inventario de equipo y preferencias.** `equipmentPreferences.js` clasifica el `equipment` (texto libre) a categorías y filtra el pool por material disponible + ejercicios excluidos, priorizando favoritos (relaja si vaciaría el pool). Cableado en `buildSessionExercises` y alternativas/swaps. Perfil: chips de "Material disponible" (vacío = sin restricción). `studio-availability` persiste `equipment`/`excludedExercises`/`favoriteExercises`; `studio-data` los expone. **Desplegado** (commit `ac1ae3e`, bundle `v=fc16703d0f`, +6 tests). UI de excluir/favorito desde el modal de ejercicio: **hecha** (`ExercisePrefActions` en `ui.jsx`; persiste vía `studio-availability` sin marcar el perfil como completo; se aplica en próximas sesiones/cambios).
...
- [x] **#6 "Por qué de tu sesión".** Tarjeta en Entreno con explicación DETERMINISTA (volumen ajustado a nivel/duración, carga por DAPRE vs fase, selección por foco/comorbilidades/material), construida desde los datos reales del usuario y su propia prescripción (`buildSessionRationale` en `studio-data`). Disciplina anti-alucinación respetada: NO afirma claims científicos sin sustento; para la base con citas remite al coach (que sí hace RAG). **Desplegado.** Citas RAG reales: **hechas** — nuevo endpoint bajo demanda `GET /api/session-rationale` que llama al retriever (`retrieveGuidelinesContextWithCitations`) y devuelve solo las fuentes realmente recuperadas; botón "Ver base científica (fuentes)" en la tarjeta. Disciplina: si no hay citas, remite al coach (no inventa). Bundle `v=c4fb58caea`.
...
- [x] **Alinear “Análisis del coach” de Progreso con el objetivo completo del usuario (desplegado 20 jun).** Integrados `goalProgress`, meta/fecha/tendencia/`onTrack`, señales de carrera (zonas, sesiones clave, eficiencia, predicción), fallbacks por objetivo y sección visible `goalAlignment`. Corregido `hrDriftBpm:null`; las cifras de recomendaciones previas se redactan. Firma de caché `v2` incluye objetivo/plan/ediciones/métricas/comidas. Incluido en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`.
...
- [x] **Movilidad/estiramiento guiado original según la sesión real.** `guidedMobility.js` selecciona por `exercise.category`, ofrece 5/10/15/20 min, cues/lado/material/seguridad y adaptaciones de embarazo/osteoporosis; `studio-data` lo expone y Entreno incluye reproductor con temporizador. Verificado localmente el 11 jul 2026 (373 tests, bundle `f46954e1e1`). Pendiente futuro: demostraciones visuales propias/licenciadas y más contextos independientes (mañana, sueño, foam roller).
- [x] **Calentamiento guiado + jerarquía de Entreno.** `buildGuidedWarmupRoutine` prepara dinámicamente los patrones reales en 5/10/15 min y añade aproximaciones en fuerza; UI prioriza calentamiento → ejercicios → enfriamiento → check-in y pliega ajustes/explicaciones/atlas/registro retroactivo. Adherencia corregida a fechas vencidas del plan semanal. Verificado localmente el 11 jul 2026 (379 tests, bundle `b6ed5014cf`).
- [ ] Mejorar correccion manual de alimentos y porciones.
- [ ] Calibrar confianza por tipo de plato.
- [ ] Ampliar historial y progresion.
- [x] **Perímetro abdominal (cintura) + riesgo cardiometabólico.** El usuario se mide la cintura y la registra (serie temporal en `metrics`, que ya soportaba `waistCm`); Progreso muestra cintura actual + **ICA (cintura/altura)** + banda por cintura/sexo (cortes IDF/NCEP, aviso de etnia) + evolución (Spark) + guía de medición. **% grasa método Navy AÑADIDO** (opcional, decisión del usuario 16 jun): `estimateBodyFatNavy` (versión métrica) — hombres cintura+cuello+altura, mujeres + cadera; etiquetado como estimación (±3-4%). En la `BiometryCard` hay una sección plegable "Estimar % grasa (Navy)" con inputs de cuello (y cadera en mujeres); `/api/metrics` y el repo guardan `neckCm`/`hipCm`; `studio-data` expone serie de % grasa. Módulo `waistRisk.js`. De paso, arreglado el bug latente de la serie de peso (`takenAt` vs `performedAt`). **Desplegado** (+8 tests).
- [x] **Recomendaciones pre/post entreno con límites clínicos.** `prePostNutrition.js` (determinista): pre (carbohidrato + proteína 1-3 h antes o snack ligero; nada si es corta/suave) y post (proteína ~0.3 g/kg acotada 20-40 g dentro del objetivo diario; reposición de carbohidrato si fue exigente; rehidratación). Cautelas por comorbilidad: diabetes → hipoglucemia antes y tardía; HTA → anti-Valsalva/cafeína. Tarjeta "Antes y después de entrenar" en Entreno. **Desplegado** (commit pendiente abajo, +6 tests). Disciplina: orientativo y acotado al objetivo diario; con condición médica, prevalece el médico.
...
- [~] Revisar y completar la sección de perfil del usuario. **Contrato de datos cerrado localmente el 20 jun:** ya no precarga persona ficticia y no permite prescripción incompleta. Sigue pendiente mejorar la UX/orden del formulario largo.
- [~] **#8 Onboarding de Perfil.** El onboarding autenticado exige objetivo, modalidad, nivel, actividad, sexo para fórmula energética, edad, peso, altura, comidas, días y minutos; salud/material/biometría/metas fechadas son opcionales. Backend devuelve campos faltantes y no activa perfil ni genera plan con supuestos. Contrato desplegado el 20 jun en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`. Pendiente de producto: convertir el formulario largo en flujo guiado multi-paso sin relajar la completitud.
- [x] **Contrato de datos reales / de-sample (desplegado 20 jun).** Separación estricta demo-autenticado, vacíos honestos, bloques stale sin reciclaje, perfil/macros/cargas sin valores personales por defecto, carga glucémica etiquetada como estimación, y dashboard legacy sin nombres/métricas/actividad de muestra. Incluido en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`; entrega acumulada con 339 tests y build verdes.
- [ ] Activar entrega automatica de alertas: bloqueado por plan Vercel `hobby`.
- [ ] Completar revision legal humana de privacidad, consentimiento y disclaimer por mercado.
