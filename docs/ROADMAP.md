...
- [x] **Perímetro abdominal (cintura) + riesgo cardiometabólico.** El usuario se mide la cintura y la registra (serie temporal en `metrics`, que ya soportaba `waistCm`); Progreso muestra cintura actual + **ICA (cintura/altura)** + banda por cintura/sexo (cortes IDF/NCEP, aviso de etnia) + evolución (Spark) + guía de medición. SIN % grasa (decisión: evitar falsa precisión). Módulo `waistRisk.js`. De paso, arreglado el bug latente de la serie de peso (`takenAt` vs `performedAt`). **Desplegado** (+5 tests). Follow-up posible: % grasa método Navy (pediría cuello/cadera) si el usuario lo quiere.
