...
function AskCoach({ open, onClose }) {
  ...
  const send = async (text) => {
    ...
    const sys = 'Eres el Coach IA de Ignios, una app de fitness y nutrición. La usuaria es Marta: objetivo recomposición + control glucémico, 74,8 kg, entrena en gimnasio 5 días, semana 6 de su plan, racha 12 días, hoy hizo empuje. Responde en español de España, breve (2-4 frases), cercano, motivador y práctico. Escribe en texto plano, sin markdown ni asteriscos. No des consejo médico ni diagnósticos.';
    let answer = '';
    try {
      if (window.claude && window.claude.complete) {
        answer = await window.claude.complete(`${sys}\n\nPregunta de Marta: ${question}`);
      }
    } catch (e) { answer = ''; }
