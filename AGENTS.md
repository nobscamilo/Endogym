...
## Estado acumulado (actualizado el 19 de junio de 2026)

- Vercel responde en `/` y `/api/health`; `/api/meals` sin token responde `401`. El 19 de junio de 2026 `endogym.vercel.app` resolvía a `dpl_yiR1GVoJnYZVdo4njGxy7yqbNwVF` (`Ready`), bundle `097b1b9fba`. Vuelve a comprobarlo antes de afirmarlo en conversaciones futuras.
...
- `main` local está alineado con `origin/main` en `8e46bd1` (19 jun 2026), el commit servido por el deployment público verificado. Quedan artefactos/logs sin commit posibles en `scratch/`, `.playwright-cli/` y `output/`; ignóralos salvo que el usuario pida limpieza.
...
- **Análisis del coach alineado con objetivos (19 jun 2026, noche-2):** corregido y verificado localmente, **no desplegado**. `coach-analysis` integra `goalProgress` (SMART/meta/tendencia/fecha/`onTrack`) y señales deterministas de carrera (FCmáx/Z2, zona real vs prescrita, sesiones clave, eficiencia y predicción); contrato/UI añade `goalAlignment` (“Consonancia con tu objetivo”). Fallback ramifica por endurance/fuerza/peso/glucémico. `hrDriftBpm:null` ya no imprime `+null`; cifras de recomendaciones IA previas se redactan para no perpetuar alucinaciones. Caché `v2` invalida por objetivo, plan, ediciones de entreno, métricas y comidas. Bundle local `29b865f9b9`; 308 tests y build OK.
