...
- **Prescripción desde Perfil/FITT-VP (15 jun 2026):** implementado y verificado localmente, **no desplegado**. Perfil guarda `trainingExperience` (`novice`/`intermediate`/`advanced`), `studio-data` lo rehidrata, y `exerciseLibrary` modula volumen/series/descanso. `generateWeeklyPlan` ya no recorta `daysPerWeek` quedándose con los primeros días: selecciona sesiones prioritarias por objetivo/modalidad (en `hybrid_run_gym` con carrera conserva tirada larga + calidad + fuerza). Se corrigieron claims internos de bibliografía (ACSM 11th/RAG local, no ACSM 12th/2026). Bundle `e102303dd5`; `check:conflicts`, `audit`, `smoke`, 247 tests, `npm run build` y Playwright local OK.
- **Cambio de grupo muscular (15 jun 2026):** implementado y verificado localmente, **no desplegado**. Entreno muestra "Grupo muscular" en sesiones de fuerza/mixtas; `POST /api/studio-swap` acepta `scope:'focus'` + `sessionFocus` (`upper`/`push`/`pull`/`lower`/`full_body`) y reconstruye solo la sesión de hoy con ejercicios, calentamiento/enfriamiento, restricciones, fase e interferencia. La regla de no repetir días seguidos vive en servidor (`buildSessionFocusChange`/`listSessionFocusChangeOptions` en `planner.js`) y bloquea focos que choquen con la sesión anterior/siguiente. Bundle `a0e5d9dc07`; `check:conflicts`, `audit`, `smoke`, 251 tests, `npm run build` y Playwright local OK.
- **Backlog de prescripción sugerido (15 jun 2026):** documentado en `docs/ROADMAP.md` y `docs/STUDIO_REDESIGN.md`: previsualizar focos bloqueados, proponer reordenar sesiones ante conflicto, check-in por grupo muscular, inventario real de equipo/preferencias, registro por serie, explicación con citas RAG y revisión mensual del mesociclo. Está **pendiente**, no lo trates como implementado.
