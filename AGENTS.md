...
1. `README.md`
2. `docs/PROJECT_STATUS.md` (las sesiones antiguas viven en `docs/PROJECT_STATUS_ARCHIVE.md`)
...
- **(17 jul 2026, parte 2) Editor retroactivo: los guardarraíles de equilibrio NO bloquean días pasados.** Cambiar el grupo muscular de un día YA OCURRIDO es registrar la realidad, no planificar. `listSessionFocusChangeOptions`/`buildSessionFocusChange` (`planner.js`) aceptan `retroactive`: en pasado, el motivo de adyacencia/sobrecarga se informa como nota pero no bloquea (`available:true`), se conserva el aviso clínico de conversión y se devuelve `retroactive:true`; hoy/futuro quedan intactos. `studio-swap` pasa `retroactive = targetDate !== today`; `session-for-date` marca `retroactive = date !== today` en `mapTodaySession` → los chips del `PastSessionLogger` se desbloquean sin tocar el bundle. Tests en `tests/core/calendar-and-exercise-metadata.test.js` (+2). **DESPLEGADO:** commit `1ae9df5` (junto con el blindaje de caché de la parte 1), 418 tests verdes en la Mac, build OK, deployment `dpl_AkJuLr2neWymaHSKA4i5QPcpHxKz` (prod, READY); alias `endogym.vercel.app` reasignado a mano. **Los 2 tests rojos de `coach-analysis` (17 jul) eran un test que se pudre con el reloj** (fixture 18-jun fuera de la ventana de 28 días de `buildCoachAnalysisDigest`), NO una regresión: se arregló congelando el reloj del test (`vi.setSystemTime`); suite 420/420 verde. Ojo con otros tests de fechas fijas.
- **(17 jul 2026, parte 1) Reporte "fuga de datos entre usuarios" NO reproducido + blindaje de caché:** el aislamiento server-side está intacto (todas las rutas usan `user.uid` del token verificado; repo scoping `users/{uid}/...`; sin uid de cliente; sin caché de módulo de datos de usuario; sin service worker; el `Cache-Control: public` de `next.config.mjs` es solo para assets de `/studio`, no `/api`). Sonda real (`scratch/user-isolation-probe.mjs`, proyecto `endogym-vtety8`): **solo 2 cuentas Auth** con datos **distintos** (gmail `weight_loss` / outlook `recomposition`), ninguna creada en julio. La causa más probable del síntoma es prueba local con `AUTH_DISABLED=true` (todos → `dev-user`); `auth.js` ya bloquea `dev-user` fuera de `NODE_ENV=development`. RAG verificado OK y diferenciado por objetivo (`scratch/rag-probe.mjs`; las sesiones las diferencia el PLANNER, el RAG solo inyecta contexto). **Hardening:** `jsonResponse` (`src/lib/http.js`) emite por defecto `Cache-Control: private, no-store` en toda la API (+`tests/lib/http.test.js`). Verificado con node (PASS); vitest/build/deploy PENDIENTES en la Mac (node_modules darwin en el sandbox). Pendiente: el usuario debe aportar los 3 emails para sondear esos uids si insiste en que probó en producción.

- Vercel responde en `/` y `/api/health`; `/api/meals` y `POST /api/coach-chat` sin token responden `401`. El 20 de junio de 2026 `endogym.vercel.app` quedó reasignado manualmente a `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h` (`Ready`), URL inmutable `endogym-9d6ey3oz8-juan-camilo-sarmientos-projects.vercel.app`, bundle `bb2659ac92`. Vuelve a comprobarlo antes de afirmarlo en conversaciones futuras.
...
- **Vídeos (20 jun 2026, desplegado):** `EXERCISE_VIDEO_MAP` conserva 55 asociaciones exactas (49 vídeos únicos), todas resolubles por YouTube oEmbed. Movimiento e implemento deben coincidir; solo se comparte ID entre alias del mismo ejercicio. `studio-data` y el dashboard resuelven el mapa vigente por `exercise.id` para no revivir asociaciones obsoletas guardadas en planes. “Sigue aprendiendo” se deriva solo de los embeds verificados de la sesión de hoy; se eliminó el feed editorial ficticio (`discover`) y la reproducción simulada. Sin vídeo exacto, la biblioteca abre una búsqueda específica real. Bundle `bb2659ac92`, 339 tests y Playwright de producción verdes.
...
- **(22 jun 2026) El working tree del 20 jun YA está commiteado y pusheado:** `main` = `origin/main` = `b43d13f` (44 archivos: bundle, rutas API, `planner.js`, `exerciseLibrary.js`, `profileCompleteness.js`, docs). Con esto **producción (`dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`, bundle `bb2659ac92`) vuelve a ser reproducible desde `origin/main`**. Quedan artefactos/logs posibles en `scratch/`, `.playwright-cli/` y `output/`; ignóralos salvo que el usuario pida limpieza.
...
- **Calentamiento/vuelta a la calma dinámicos (11 jun 2026, noche-2):** `src/core/warmupCooldown.js` ensambla OBLIGATORIO en cada sesión: general (termorregulación, gradual con HTA, sin impacto con artrosis), específico (movilidad+activación por foco del día, zonas lesionadas antes de cargar, anti-Valsalva con HTA) y retorno (prolongado con HTA: nunca parar en seco; recordatorio educativo con diabetes). `detectComorbidities` es el parser léxico del perfil. NO reintroducir protocolos estáticos; al tocar la encuesta de perfil recuerda que `medicalConditions`/`physicalInjuries`/`profile.conditions` alimentan esto.
- **Calentamiento/enfriamiento DIRIGIDO por usuario (22 jun 2026, bundle `74714d1b79`):** los builders reciben ahora `exercises` y derivan los grupos/patrones REALES del día desde `exercise.category` vía `deriveWorkedGroups` (el catálogo runtime NO tiene `primaryMuscles`: 0/244). El enfriamiento nombra los grupos trabajados; el calentamiento prepara los patrones presentes; `mindbody`/`recovery` tienen bloque propio. `detectComorbidities` añade asma (general ≥10 min), embarazo (anti-Valsalva, sin supino prolongado), gravedad de HTA (`hypertensionControlled`) y cubre TODAS las zonas lesionadas priorizando la región del día. **`studio-data.mapTodaySession` recomputa warmup/cooldown del día en LECTURA** (perfil+ejercicios+fatiga `gentle`) sin exigir "Regenerar"; los días de carrera conservan el guardado (drills). Encuesta de Perfil ampliada (Asma/Embarazo/HTA controlada). 349 tests. Commit `70d68e4`, desplegado en `endogym-c43k1abv5-…`, alias manual `endogym.vercel.app` (sondas `/` 200, health 200, meals/coach-chat 401, bundle 200).
...
- **FASE 3 parcial (12 jun 2026):** registro "Hecho según plan" (1 tap, valores prescritos), feedback 👍👎 (`/api/coach-feedback`, solo hash, jamás contenido) y observabilidad de IA (`src/lib/aiMetrics.js` → colección `aiMetrics/{día}`, best-effort, sin PII; al instrumentar un endpoint nuevo, mockear aiMetrics en sus tests). 239 tests; bundle `bec6d9a891` desplegado y verificado. Quedan 3.2 push (necesita FCM/VAPID del usuario), 3.3 offline y 3.5 onboarding. Tests con fechas: usar 00:00Z y `dateKeyInTimeZone()`, no mediodía ni UTC directo (flaky por hora).
- **Consolidación (12 jun 2026, mañana-2):** fecha civil unificada en weekly-plan/coach-chat/overlay (pasar `today` civil al overlay); comorbilidades estructuradas en `profile.conditions` (la encuesta de Perfil tiene checkboxes de Salud — fuente principal de `detectComorbidities`); e2e:production verde y sondas reales de memoria/loop/DAPRE documentadas. 242 tests.
...
- **Prescripción desde Perfil/FITT-VP (15 jun 2026):** desplegada. Perfil guarda `trainingExperience` (`novice`/`intermediate`/`advanced`), `studio-data` lo rehidrata, y `exerciseLibrary` modula volumen/series/descanso. `generateWeeklyPlan` selecciona sesiones prioritarias por objetivo/modalidad al aplicar `daysPerWeek` (en `hybrid_run_gym` con carrera conserva tirada larga + calidad + fuerza).
- **Cambio de grupo muscular (15–19 jun 2026):** desplegado. `POST /api/studio-swap` reconstruye el foco y mantiene guardarraíles de adyacencia/volumen; desde el 19 jun también convierte con aviso días no-fuerza a fuerza (`effectiveType='resistance'`). Bundle `097b1b9fba`.
- **Backlog de prescripción del 15 jun:** cerrado en sus ítems principales (matriz de focos, intercambio vecino, molestias por zona, equipo/preferencias, registro por serie, explicación+citas RAG y revisión del mesociclo). Los límites futuros están en `docs/ROADMAP.md`.
- **Análisis del coach alineado con objetivos (19–20 jun 2026):** corregido, verificado y desplegado en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`. `coach-analysis` integra `goalProgress` (SMART/meta/tendencia/fecha/`onTrack`) y señales deterministas de carrera (FCmáx/Z2, zona real vs prescrita, sesiones clave, eficiencia y predicción); contrato/UI añade `goalAlignment` (“Consonancia con tu objetivo”). Fallback ramifica por endurance/fuerza/peso/glucémico. `hrDriftBpm:null` ya no imprime `+null`; cifras de recomendaciones IA previas se redactan para no perpetuar alucinaciones. Caché `v2` invalida por objetivo, plan, ediciones de entreno, métricas y comidas.
- **Editor retroactivo flexible + convención de cargas (15 jul 2026):** "Registrar otro día" permite cambiar el grupo muscular del día pasado (reescribe ESE día del bloque vía `POST /api/studio-swap` con `date` ≤14 días — decisión explícita del usuario; solo `scope:'focus'`, sin reschedule, guardarraíles intactos), añadir/quitar ejercicios (buscador de `GET /api/exercise-catalog`, catálogo compacto bajo demanda) y editar series/kg/reps; todo colapsado tras un botón. `session-for-date` mapea NO-exacto (devuelve días de descanso con `isRestDay`/`focusOptions`). **Convención de cargas:** `loadKg` = peso TOTAL (barra INCLUIDA; mancuernas = por mancuerna; máquina = placa). El desglose visual vive en `public/studio/app/studio/load-format.js` (`describeLoad`/`classifyEquipment`; el build lo concatena quitando `export`) y usa `profile.barbellKg` (5-35 kg, opcional en Perfil; default 20). NO cambiar esta semántica sin migrar historial/e1RM/DAPRE. `mapTodaySession` expone `item.equip`.
- **Contrato de datos reales (20 jun 2026, desplegado):** una sesión autenticada nunca hereda el dataset demo: `studio-data` reemplaza secciones completas y devuelve `mode`/`dataStatus`/`planStatus`, vacíos honestos y `null` cuando no hay datos. Bloques vencidos no reciclan su primer día; `studio-swap` exige que hoy exista. Perfil/onboarding ya no precargan edad/peso/altura/sexo/objetivo/experiencia: `profileCompleteness.js` exige objetivo, modalidad, nivel, actividad, sexo, edad, peso, altura, comidas, días y minutos antes de activar `studioAvailability` o generar el plan. Sin peso/historial no se estima carga desde 75 kg. Glucemia se presenta como **carga glucémica estimada**, no como medición. El dashboard legacy también quedó sin nombres, métricas, gráficas ni entrenos de muestra. Incluido en `dpl_FpbL91Ukd97dy9aT73iAwX8rh52h`; verificación pública/visual verde, sin repetir todavía el E2E autenticado integral.
...
Studio = bundle React **pre-compilado** con esbuild (`scripts/build-studio.mjs` → `public/studio/app/studio.bundle.js`, artefacto commiteado; esbuild NO es dependencia, ver `docs/STUDIO_REDESIGN.md`). El bundle se sirve como asset estático en `/studio/app/index.html` (lo embebe el iframe de "/"). Endpoints propios: `/api/studio-data` (datos reales: perfil, hoy, semana, biblioteca, macros, glucemia, progreso y overlay adaptativo del bloque activo), `/api/coach-chat` (coach IA con perfil real y rate limit persistente), `/api/studio-nutrition` (**plan semanal de comidas con Gemini: 7 días `days[]` + compra semanal + batch; `maxDuration=60`; validación per-day de macros antes de guardar; cache invalidado por `planSignature`**), `/api/analyze-plate` (foto del plato → macros/glucemia, registra la comida), `/api/studio-availability` (encuesta), `/api/studio-swap` (cambiar ejercicios, ampliar sesión o cambiar foco muscular con guardarraíl de días adyacentes; **desde jun 2026 el cambio de foco también CONVIERTE días no-fuerza —cardio/carrera/recuperación— en fuerza** vía `effectiveType='resistance'` en `planner.buildSessionFocusChange`, devolviendo `converted`+`warning` clínico y manteniendo los guardarraíles; la reprogramación por intercambio sigue siendo solo para días de fuerza/mixto), `/api/session-for-date` (**registro retroactivo**: GET de la sesión prescrita de una fecha pasada ≤14 días, validada y no futura, para registrar/editar un día previo; reusa `mapTodaySession(..., { exact:true })` con `dateKeyInTimeZone()` como "hoy"), `/api/public-config`. El registro retroactivo escribe vía `POST /api/workouts` con `performedAt`/`dailyCheckinDate` de la fecha e `id` determinista `manual-{fecha}` (idempotente, no duplica). CSP estricta global + relajada scoped a `/studio*` (assets del iframe) en `next.config.mjs`. **Tras editar `public/studio/app/studio/*` o `scripts/build-studio.mjs` hay que regenerar el bundle (`npm run build:studio`) y commitearlo.** Detalle en `docs/STUDIO_REDESIGN.md`.
