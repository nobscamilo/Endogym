...
def flood_fill_background(img, tolerance=10):
    """Robust queue-based flood fill to remove white background without touching internal highlights."""
    rgba = img.convert('RGBA')
    width, height = rgba.size
    pixels = rgba.load()
    
    # We flood fill from all 4 corners
    ...
    while queue:
        ...
        # Check if pixel color is close to white background within tolerance
        if abs(r - bg_color[0]) <= tolerance and abs(g - bg_color[1]) <= tolerance and abs(b - bg_color[2]) <= tolerance:
            # Make it fully transparent white
...
def main():
    ...
    if not old_base_path.exists():
        print("Error: old-base.png not found. Make sure you restored it.")
        sys.exit(1)
    ...
    # 2. Load the user's new gray images
    gray_front_path = ANATOMY / 'gray-front.png'
    gray_back_path = ANATOMY / 'gray-back.png'
    
    if not gray_front_path.exists() or not gray_back_path.exists():
        print(f"Error: gray-front.png or gray-back.png not found in {ANATOMY}")
        sys.exit(1)
    ...
    gray_front_clean = flood_fill_background(gray_front_raw, tolerance=15)
    gray_back_clean = flood_fill_background(gray_back_raw, tolerance=15)
